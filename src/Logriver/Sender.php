...
use Logriver\ErrorHandler as ErrorHandler;
...
abstract class Sender
{
    ...
    public function doCaptureElement($type, $messageOrObject, $mixedData = null)
    {
        ...
        if ($type >= 1 && $type <= 5) {
            ...
            elseif(is_string($messageOrObject)) {
                ...
                $traceFound = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
                for($i=0;$i<=1;$i++) {
                    if (isset($traceFound[$i]['class']) && ($traceFound[$i]['class'] == 'Logriver\Sender' || $traceFound[$i]['class'] == 'Logriver_Sender')) {
                        unset($traceFound[$i]);
                    }
                }
    ...
    private function doSendData($data)
    {
        ...
        $socket = stream_socket_client('ssl://' . $host.":443", $errno, $errstr, 3, STREAM_CLIENT_CONNECT, $context);
        ...
        if ($socket !== false) {
            ...
            if (function_exists('curl_init')) {
                // With Php 5.6 ssl:// doesn't work
                $url = 'https://' . $host . ':443/traxer/add/';
                $ch = \curl_init();
